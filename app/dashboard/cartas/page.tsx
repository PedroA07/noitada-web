...
const Icons = {
  ...
  Masculino:() => <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4" strokeLinecap="round"><circle cx="6" cy="10" r="4.5" stroke="#60A5FA" strokeWidth="1.5"/><line x1="9.5" y1="6.5" x2="14" y2="2" stroke="#60A5FA" strokeWidth="1.5"/><polyline points="11,2 14,2 14,5" stroke="#60A5FA" strokeWidth="1.5"/></svg>,
  Feminino: () => <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4" strokeLinecap="round"><circle cx="8" cy="6" r="4.5" stroke="#F472B6" strokeWidth="1.5"/><line x1="8" y1="10.5" x2="8" y2="14" stroke="#F472B6" strokeWidth="1.5"/><line x1="5.5" y1="12.5" x2="10.5" y2="12.5" stroke="#F472B6" strokeWidth="1.5"/></svg>,
...
const ICON_RARIDADE: Record<string, () => JSX.Element> = {
  comum: Icons.Comum, incomum: Icons.Incomum, raro: Icons.Raro,
  epico: Icons.Epico, lendario: Icons.Lendario,
};
...
type FormCarta = {
  nome: string; personagem: string; vinculo: string;
  categoria: string; raridade: string; genero: string;
  imagem_url: string | null; descricao: string | null;
};
const VAZIA: FormCarta = {
  nome:'', personagem:'', vinculo:'',
  categoria:'anime', raridade:'comum', genero:'outros',
...
export default function CartasPage() {
  ...
  const salvar = async () => {
    ...
    try {
      ...
      const pontuacao = calcPts(form.raridade, form.nome, form.vinculo);
      const payload   = { ...form, imagem_url: imagemUrl, imagem_r2_key: imagemR2Key, criado_por: session.user.id, pontuacao };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <style>{`
        @keyframes s-in{from{opacity:0;transform:translateY(-6px)}to{opacity:1;transform:translateY(0)}}
        .s-drop{animation:s-in .15s ease}
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex-1 min-w-60 relative">
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar por nome, personagem ou vínculo..."
          ...
          {cartas.map(carta => {
            ...
            return (
              <div key={carta.id} className={`relative border-2 rounded-2xl overflow-hidden bg-black/40 transition-all hover:scale-[1.03] hover:shadow-xl ${m.border} ${m.bg}`}>
                <div className="relative w-full aspect-[3/4] bg-gray-900/60">
                  {carta.imagem_url
          ...
          <div className="relative bg-[#07070F] border border-white/[0.08] rounded-3xl w-full max-w-[1120px] shadow-[0_32px_80px_rgba(0,0,0,0.9)] flex flex-col max-h-[92vh]">
            ...
            <div className="flex flex-1 min-h-0 overflow-hidden divide-x divide-white/[0.07]">
                    ...
                    {optsGenero.map(g => (
                      <button key={g.valor} type="button" onClick={() => setForm(f => ({ ...f, genero: g.valor }))}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${
                          form.genero === g.valor ? 'bg-fuchsia-500/20 text-fuchsia-400 border-fuchsia-500/50' : 'bg-black/40 border-white/10 text-gray-500 hover:border-white/20 hover:text-white'
                        }`}>
                        <g.Icon />{g.label}
                      </button>
                    ))}
                ...
                <div>
                    ...
                    {(['url', 'upload'] as const).map(modo => (
                      <button key={modo} type="button" onClick={() => setModoImagem(modo)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${
                          modoImagem === modo ? 'bg-fuchsia-500/20 text-fuchsia-400 border-fuchsia-500/50' : 'bg-white/5 border-white/10 text-gray-500 hover:text-white'
                        }`}>
                  ...
                  {modoImagem === 'url' ? (
                    <input value={form.imagem_url || ''} onChange={e => { setForm(f => ({ ...f, imagem_url: e.target.value })); setPreviewImagem(e.target.value || null); }}
                      ...
                      placeholder="https://pub-xxx.r2.dev/cartas/..." />
                  ) : (
                    <>
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center justify-start gap-5 custom-scrollbar">
                  <PreviewCard form={form} img={previewImagem} />
                  <div className="w-full max-w-[196px] grid grid-cols-2 gap-2">
                    {[
                      { label: 'Spawn',    value: `${meta.peso}%` },
                      { label: 'Pts base', value: meta.ptsBase.toLocaleString('pt-BR') },
                    ].map(i => (
                ...
                <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 custom-scrollbar">
                  ...
                  <div className="px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                    <p className="text-[9px] text-gray-600 leading-relaxed">
                      Visualização de como a carta aparece no Discord quando o bot <span className="text-fuchsia-400/80 font-bold">Lua</span> envia via <code className="font-mono text-gray-500">/roll</code>. Os emojis são nativos do Discord.
                    </p>
                  </div>
                </div>
              </div>
