...
export default function CartasPage() {
  ...
  const abrirModal = (carta?: Carta) => {
    if (carta) {
      ...
      setForm({
        nome: carta.nome,
        personagem: carta.personagem,
        vinculo: carta.vinculo,
        categoria: carta.categoria,
        raridade: carta.raridade,
        genero: carta.genero || 'outros',
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          ...
          {GENEROS.map(g => (
            <option key={g} value={g}>{EMOJI_GENERO[g]} {g.charAt(0).toUpperCase() + g.slice(1)}</option>
          ))}
        </select>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500">Nenhuma carta encontrada.</div>
      ) : (
            ...
            >
              <div className="relative w-full aspect-[3/4] bg-gray-900">
                ...
                <div className="absolute top-2 right-2 flex flex-col items-end gap-1">
                  <span className="text-lg">{EMOJI_RARIDADE[carta.raridade]}</span>
                  <span className="text-sm">{EMOJI_GENERO[carta.genero] || '⚧️'}</span>
                </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-gray-900 border border-white/10 rounded-3xl w-full max-w-2xl p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-400 font-black uppercase tracking-widest block mb-1">
                    Nome da Carta *
                  </label>
                  <input
                    value={form.nome}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={form.raridade}
                    onChange={e => setForm(f => ({ ...f, raridade: e.target.value }))}
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:border-fuchsia-500 outline-none appearance-none"
                  >
                    {RARIDADES.map(r => (
                      <option key={r} value={r}>
                        {EMOJI_RARIDADE[r]} {r.charAt(0).toUpperCase() + r.slice(1)}
                      </option>
                    ))}
                  </select>
                </div>
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all border ${
                        form.genero === g
                          ...
                          : 'bg-black/40 border-white/10 text-gray-300 hover:border-white/30 hover:text-white'
                      }`}
                    >
                      {EMOJI_GENERO[g]} {g}
                    </button>
              ...
              {msg && (
                <p className={`text-sm font-bold ${msg.startsWith('✅') ? 'text-green-400' : 'text-red-400'}`}>
                  {msg}
                </p>
              )}
