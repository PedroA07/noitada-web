...
export const obterMembrosComStatus = async () => {
  try {
    ...
    const { data: configDB } = await supabase.from('configuracoes_servidor').select('*').eq('guild_id', guildId).single();
    const { data: perfisDB } = await supabase.from('perfis').select('discord_id, status, avatar_url, nome');
...
export const sincronizarCargoDiscord = async (membroId: string, cargoId: string, adicionar: boolean) => {
  try {
...
export const moderarMembro = async (membroId: string, acao: 'kick' | 'ban' | 'mute' | 'nickname', dados?: any) => {
  try {
    await fetch('/api/discord/moderacao', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
